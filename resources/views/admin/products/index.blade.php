...
@section('title', settings()->website_title . ' | ' . __('words.products'))
@section('breadcrumb')
    <div class="d-flex align-items-baseline flex-wrap mr-5">
        ...
        <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
            ...
            <li class="breadcrumb-item">
                <span class="text-muted">{{ __('words.show_products') }}</span>
            </li>
        </ul>
        <!--end::Breadcrumb-->
    </div>
                            ...
                            <td>
                                @if (!$product->image)
                                    <a href="{{ asset('uploads/default_image.png') }}" data-toggle="lightbox"
                                        data-title="{{ $product->title }}" data-gallery="gallery">
                                        <img class="index_image" src="{{ asset('uploads/default_image.png') }}"
                                            onerror="this.src='{{ asset('uploads/default_image.png') }}'"
                                            alt="default-image">
                                    </a>
                                @else
                                    <a href="{{ $product->image }}" data-toggle="lightbox"
                                        data-title="{{ $product->title }}" data-gallery="gallery">
